...
(function($) {
    ...
    var extensionMethods = {
        ...
        pips: function( settings ) {
                ...
                options = {
                    ...
                    formatLabel: function(value) {
                        try {
                            ...
                            console.error(ex);
                            console.log('--Exception with the following value: ', value);
                            return ' ';
                        }
                    }
                };
            ...
            slider.options.pipStep = slider.options.pipStep;
        ...
        float: function( settings ) {
            ...
            slider.element.on( options.event , function( e, ui ) {
                ...
                $(ui.handle).find(".ui-slider-tip")
                    .css('position', 'fixed')
                    .css('top', $(ui.handle).offset().top - 30)
                    .css('left', $(ui.handle).offset().left + 8.85) // 1/2 of 1.2 em
                    .html( options.formatLabel(val))
                ;
//                console.log('e', e);
            });

            // Added by SWR: Change the position
            slider.element.on( "slidestop" , function (e, ui) {
                $(ui.handle).find(".ui-slider-tip")
                    .css('position', 'fixed')
                    .css('top', $(ui.handle).offset().top - 30)
                    .css('left', $(ui.handle).offset().left + 8.85) // 1/2 of 1.2 em
...
//            slider.element.on( "mouseover", function (e, ui) {
//                console.log('mouse over event');
//            });

            slider.element.on( "slide" , function (e, ui) {
                $(ui.handle).find(".ui-slider-tip").hide();
            });
            // (/Added by SWR)
